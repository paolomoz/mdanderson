...
export async function dismissOverlays(page, { extra = [], lateWindowMs = 6000 } = {}) {
  ...
  // (c) iframe-hosted survey invites (Verint/ForeSee — recorded:
  // mdanderson.org "We welcome your feedback!"). The invite's decline control
  // lives INSIDE the vendor iframe, unreachable by page.locator (main-frame
  // only) — sweep every frame for an exact-text decline control. Learnings
  // candidate: upstream as overlay class (c).
  const declineSurveyAllFrames = async () => {
  ...
  if (await declineSurveyAllFrames()) {
    dismissed.marketing.push('iframe-survey-decline');
    await page.waitForTimeout(600);
    // prune any survey wrapper/scrim left in the main frame
    await page.evaluate(() => {
      document.querySelectorAll('#acsMainInvite, .__acs, [id^="fsr"], .fsrDialog, .fsrOverlay, iframe[src*="verint" i], iframe[src*="foresee" i]')
        .forEach((n) => { const w = n.closest('[class*="invite" i], [id*="invite" i]') || n; w.remove(); });
    }).catch(() => {});
  }
