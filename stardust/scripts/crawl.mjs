...
async function dismissSurveyInvite(page) {
  // poll ~16s: the invite fires on a timer after load; it may render inside a
  // Verint iframe, so search EVERY frame, not just the main document.
  const tryDismissInFrame = (frame) => frame.evaluate(() => {
    const sels = ['#acsInviteNoButton', '.fsrDeclineButton', '#fsrInvite .fsrButton--decline'];
    for (const s of sels) { const el = document.querySelector(s); if (el) { el.click(); return true; } }
    const btns = [...document.querySelectorAll('[role="dialog"] a, [role="dialog"] button, .__acs a, .__acs button, [id*="Invite" i] a, [id*="Invite" i] button, a, button')];
  ...
  for (let i = 0; i < 8; i += 1) {
    ...
    for (const frame of page.frames()) {
      ...
      if (await tryDismissInFrame(frame)) { clicked = true; break; }
    }
    if (clicked) {
      await page.waitForTimeout(400);
      // prune any survey container + scrim left behind
      await page.evaluate(() => {
        document.querySelectorAll('#acsMainInvite, .__acs, [id^="fsr"], .fsrDialog, .fsrOverlay, iframe[src*="verint" i], iframe[src*="foresee" i]')
          .forEach((n) => n.remove());
      }).catch(() => {});
      await page.mouse.move(4, page.viewportSize() ? page.viewportSize().height - 4 : 896);
      return;
    }
...
async function capturePage(context, url, slug, args) {
  ...
  // Verint/ForeSee survey invite fires on a timer ("We welcome your feedback!",
  // mdanderson.org) — poll for it and decline, then park the pointer so no
  // :hover state leaks into the capture.
  if (args.consent) await dismissSurveyInvite(page);
